...
public class ProjectorSim : MonoBehaviour
{
    ...
    void Awake ()
    {
        ...
        if (EditorApplication.isPlaying)
        {
            ...
            cookies[0] = new Cookie(new CookieData(shift_v, shift_h, keystoneH, keystoneV, throwRatio, aspectRatio), cookieSizeInt, lights[0], lights[1], lights[2], forceLegacyMethod, images[0], colour);
            ...
            for (int i = 1; i < cookies.Length; i++)
            {
                if (forceLegacyMethod) // revert to old way of re-using first cookie's texture
                    ...
                    cookies[i] = new Cookie(new CookieData(shift_v, shift_h, keystoneH, keystoneV, throwRatio, aspectRatio), cookieSizeInt, lights[0], lights[1], lights[2], false, images[i], colour);
            }            
        ...
        else // EDITOR EDIT MODE - only calculate the preview cookie
        {
            ...
            cookies[0] = new Cookie(new CookieData(shift_v, shift_h, keystoneH, keystoneV, throwRatio, aspectRatio), cookieSizeInt, lights[0], lights[1], lights[2], forceLegacyMethod, images[previewImageIndex], colour);
        }
#endif
        AssignLightCookies();
    }
